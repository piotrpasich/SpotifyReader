...
export type ITrackState = IPaginationBaseState;
...
export class Track extends React.Component<ITrackProps, ITrackState> {
  ...
  render() {
    const { trackList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="track-heading">
          Tracks
        ...
        <div className="table-responsive">
          {trackList && trackList.length > 0 ? (
            <Table responsive aria-describedby="track-heading">
              <thead>
                <tr>
              ...
              <tbody>
                {trackList.map((track, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{track.name}</td>
                    <td>{track.url}</td>
                    <td>{track.externalId}</td>
                    <td>{track.album ? <Link to={`album/${track.album.id}`}>{track.album.name}</Link> : ''}</td>
            ...
            <div className="alert alert-warning">No Tracks found</div>
          )}
        </div>
        <div className={trackList && trackList.length > 0 ? '' : 'd-none'}>
